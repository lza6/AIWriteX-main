    ...
    async def get_news_list(self, category: dict = None) -> List[Dict]:
        ...
        try:
            ...
            for a in articles[:20]:
                ...
                if title and len(title) > 5 and href and ('war.163.com' in href or href.startswith('/')):
                    if href.startswith('/'):
                        href = 'https://war.163.com' + href
                    result.append({
                        "title": title,
                        "article_url": href,
            ...
            logger.error(f"获取网易军事新闻列表失败: {e}")
            return []

    async def get_news_info(self, item: dict, category: str = "军事") -> Dict:
        """获取新闻详情"""
        try:
