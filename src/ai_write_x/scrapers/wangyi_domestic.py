...
class WangYiDomestic(BaseSpider):
    """网易国内新闻爬虫"""
    source_name = "网易国内新闻"
    category = "国内"
    ...
    async def get_news_list(self, category: dict = None) -> List[Dict]:
        ...
        try:
            ...
            if not articles:
                articles = content_html.xpath('//div[@class="item_bt"]//a')
            if not articles:
            ...
            for a in articles[:20]:
                ...
                if title and len(title) > 5 and href and ('news.163.com' in href or href.startswith('/')):
                    if href.startswith('/'):
                        href = 'https://news.163.com' + href
                    result.append({
                        "title": title,
                        "article_url": href,
            ...
            logger.error(f"获取网易国内新闻列表失败: {e}")
            return []

    async def get_news_info(self, item: dict, category: str = "国内") -> Dict:
        """获取新闻详情"""
        try:
